...
func TestFilterProtectedProcesses_SkipsCurrentProcessAndAncestors(t *testing.T) {
	...
	filtered, err := filterProtectedProcesses(procs, wt, 200, parentMap(parents), cwdMap(cwds))
	if err != nil {
...
func TestFilterProtectedProcesses_ReturnsErrorWhenParentLookupFails(t *testing.T) {
	...
	filtered, err := filterProtectedProcesses(procs, wt, 200, func(pid int32) (int32, error) {
		if pid == 200 {
...
func TestHasOutsideCwdAncestor(t *testing.T) {
	...
	if !hasOutsideCwdAncestor(10, wt, map[int32]struct{}{}, parentMap(parents), cwdMap(cwds)) {
		t.Fatal("expected outside ancestor for pid 10")
	}
	if hasOutsideCwdAncestor(20, wt, map[int32]struct{}{}, parentMap(parents), cwdMap(cwds)) {
		t.Fatal("orphaned pid 20 should have no outside ancestor")
	}
	if hasOutsideCwdAncestor(30, wt, map[int32]struct{}{}, parentMap(parents), cwdMap(cwds)) {
